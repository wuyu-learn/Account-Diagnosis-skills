...
| ACC-03 | 指定基金的某个具体份额（如定投计划某笔）的详情 | 可选 `serialNo`、`uri` |
| ACC-19 | 某个产品（基金/投顾/货币/专户等）的份额明细 | 可选 `productId` |
| ACC-04 | 投顾账户整体、组合数量或账户收益 | `{}` |
| ACC-05 | 指定投顾组合的某个具体份额的详情 | 可选 `serialNo`、`uri` |
| ACC-06 | 指定投顾组合某个具体份额的调仓记录 | 可选 `serialNo`、`uri` |
| ACC-07 | 货币账户、货币基金账户或现金宝整体 | `{}` |
| ACC-08 | 指定货币基金的某个具体份额的详情 | 可选 `serialNo`、`uri` |
| ACC-09 | 专户账户整体 | `{}` |
| ACC-10 | 指定专户产品的某个具体份额的详情 | 可选 `serialNo`、`uri` |

卡片分三层：账户级（ACC-01/02/04/07/09）、产品级（ACC-19，跨类型通用）、份额级（ACC-03/05/06/08/10，类型专属）。优先选择最具体的层级；无法确定账户类型时选择 ACC-01；未指定产品时不得选择产品级或份额级卡片，未指定具体份额时不得选择份额级卡片。
...
- Planner 只用用户明确提供的实体填参数，未提供时保持空对象，不得猜测；
- ACC-19 可选 `productId`；ACC-03/05/06/08/10 可选 `serialNo`、`uri`；其余未列出的卡片参数必须为空。

### 产品级 `productId` 填值

ACC-19 是产品级卡片，`productId` 是产品代码（跨类型通用），不是基金代码。上游/实体已提供 `productId` 时，Planner 直接透传到 `params.productId`；不自行从基金代码换算，不猜测；未提供时保持空对象。

### 份额级 `serialNo`/`uri` 填值

ACC-03/05/06/08/10 是份额级卡片，`serialNo`/`uri` 定位某一笔具体份额。份额是人工定义的单位，比渠道更细（产品→渠道→份额）。Planner 在 plan 阶段拿不到份额标识，`params` 保持空对象；标识在 MCP 之后由份额匹配步骤取得，取数路径为：
...
`serialNo`/`uri` 是份额定位标识，是"MCP 数据不写入 `cards[].data`"的唯一例外（见 `mcp-evidence.md`）；其余 MCP 字段仍不入卡。份额比渠道细，只给渠道可能命中多笔，消歧字段待联调确认。
